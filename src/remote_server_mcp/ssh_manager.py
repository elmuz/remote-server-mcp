"""
SSH Connection Manager for Remote Server MCP
...
class SSHManager:
    ...
    async def connect(self) -> None:
        ...
        connect_kwargs = {
            ...
            "port": port,
            "username": username,
            # Disable known_hosts checking to avoid issues with:
            # - ML-KEM/post-quantum key exchange algorithms
            # - DSA keys with non-standard key lengths
            # - Other key format incompatibilities
            # Security trade-off: Accepts any host key (first-use trust)
            "known_hosts": None,
        }

        # Authentication: prefer key, fallback to password
        if key_path:
            # Expand ~ in key path
            import os
        ...
        try:
            ...
            logger.error(
                f"Connection kwargs: host={host}, port={port}, "
                f"username={username}, key_path={key_path}"
            )
            raise
    ...
    async def execute_safe_command(self, command: str, timeout: int = 30) -> str:
        ...
        This method ONLY executes commands constructed by the application,
        never raw user input. All commands must pass security checks.
        ...
        Returns:
            Command output
        """
        # Ensure command comes from application code, not user
        ...
        # Type guard: connection must be set after connect()
